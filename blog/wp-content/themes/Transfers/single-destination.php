...
if ( have_posts() ) {
	...
	<?php
		...
		$connecting_destination_results = $transfers_plugin_post_types->list_connecting_destinations('destinations2.post_title', 'ASC', $destination_obj->get_id());
				...
				<table class="hover">
					...
					<?php
							...
							foreach ($connecting_destination_results as $available_result) { 
								...
								$search_url = $advanced_search_url . "?ppl=1&trip=1&p1=" . $destination_obj->get_id() . "&dep=" . date(TRANSFERS_PHP_DATE_FORMAT_NO_TIME, $tomorrow);
					...
					<tr>
						...
						<td>
							<a href="<?php echo esc_url($search_url .= '&d1=' . $available_result->destination_to_id); ?>">
						<?php
