...
<?php
    ...
    if (!empty($_GET['station'])) {
        ...
        $allLinesID = $db->get_results("select line_tid from tfl_line_routes WHERE originator='$exist_station->code' OR destination='$exist_station->code'");
        if (!empty($allLinesID)) {
            $lines4 = array();
            foreach ($allLinesID as $tmp4) $lines4[] = $tmp4->line_tid;
            $lines4_txt = implode(",",$lines4);
            $allLines = $db->get_results("select * from tfl_lines where tid in ('$lines4_txt') order by name asc");
        }
    } else {
        $allLines = $db->get_results("select * from tfl_lines order by name asc");
    }
...
?>
    ...
    <div class="form-row">
        <div class="col">
            <input type="text" class="form-control form-control-lg" placeholder="Station" name="station"  required="required" value="<?=$exist_station->name?>">
        </div>
        <div class="col">
            <select class="" >
            <?php
              foreach ($allLines as $line3) {
                  if ($_GET['line'] == $line3->tid) echo "<option selected=\"selected\" value=\"$line3->tid\">$line3->name ($line3->modeName)</option>\r\n";  else echo "<option value=\"$line3->tid\">$line3->name ($line3->modeName)</option>\r\n";
              }
            ?>
            </select>
        </div>
